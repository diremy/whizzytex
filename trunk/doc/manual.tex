...
the \href{http://pauillac.inria.fr/advi/}{\ActiveDVI} viewer will
...
\section{Per document customization} %

\subsection{Emacs customization}
...
A per document customization is also possible by 
including a \lst".whizzytexrc" configuration file in 
...
\begin{quote}
\em
This feature requires version \texttt{1.50+3} or more recent of
{\ActiveDVI}. 
\end{quote}
...
Active-DVI provides a notion of active boxes (specified with advi
\lst"\special" annotations.  When {\ActiveDVI} is put in edition mode,
...
%
This dream has now become real...

However, \WhizzyEdit ing is not meant to break up the structual edition
principle of {\TeX} and {\LaTeX}. Its incremental viewing is an assistance
to source edition. Similarly, mouse editing should also be seen as an
assistance to and not a replacement of strutural edition.  In particular,
all editions are visible in the emacs source buffer, can be saved, manually
changed or disable. Moreover, Whizzy-editing is not meant for document
layout, but to the cases where some dimensions cannot be structurally
computed and need manual tuning. 

For instance, think of importing a picture and placing some bubble whose
origin must be precisely be placed above the head of some character: you'd
better do it with the mouse rather than by small measurements or random
adjustements. Drawing a graph with a few nodes may now become more
confortable with PStricks remaining within {\LaTeX} rather than using some
external tool. Finally, Whizzy-editing is likely to be more useful when
writting slides, where many visual gadgets may be used. For instance,
adjusting bubbles with the mouse is likely to be more efficient than doing
it by hand.
...
\adviedit{x=0.2139,y=-0.1348,w=0.7020}
...
The green line attached to left hand-side of the baseline on the one hand
is attached to the anchor of this object on the other hand, which is itself
drawn as a small green circle. 
}}\box0\pnode{Q}%
...
\setbox0=\hbox{\adviedit{d=0.2225}{\vtop{\vspace{\advid}}}}%
\wd0=1ex\ht0=1ex
...
\hspace{1.5em}%
Note that the anchor of the vertical space is exactly at the left-hand side
of the base line because it is unmovable. In this case, the object takes its
real dimensions.  In contrast, the parbox above, which has been moved,
behaved as a empty box.  }}}
...
\item \textbf{resize} them, using the right button.
\end{itemize}
...
Thus, for horinzontal spaces, one need and explicit \texttt{\string\vbox}:
\begin{verbatim}
...
\begin{verbatim}
        \adviedit{w}{\parbox[c]{text material}}
\end{verbatim}
...
\texttt{\string\adviy},
\texttt{\string\adviw},
\texttt{\string\advih}, and 
\texttt{\string\advid} that contain the float ratio of 
...
\makeatother
\vbox{\vspace{2em}
\hbox{\editbubble{w=2.4162,h=0.4335}
{\editbubble{w=0.3121,h=1.1212}
            {\editbubble{w=-1.7516,h=0.5145}{anchor}{first}}
            {second}}
          {third}
}}
\end{toimage}\imageflush
