...
\end{verbatim}
\end{quote}
This will install create a link \lst"Makefile" in your parent directory
to the file \lst"whizzytex/Makefile.pkg". This assumes that you have not
rename the parent directory after installation, which should remain
\lst"whizzytex". 

If you wish to edit this file, you should copy it instead of creating
a link, since upadtes may update (hence override) the file
\lst"whizzytex/Makefile.pkg" itself. 
...
\end{verbatim}
\end{quote}
Respectively, to check for newer version and download if nevessary, unpack
the newest version, build the current version, and install the current version
...
\end{verbatim}
\begin{quote}
as a show-cut \lst$"make update unpack build"$
\end{quote}
\begin{verbatim}
...
which is recommended before an updare, in case you want to restore the old
version later. 
\end{quote}
\end{quote}
The upgrading may ask questions to the users. To be fully automatic, you can
also pass the option \lst"FORCE=true", which will let the installation make
(hopefully reasonable) choices in your place.
