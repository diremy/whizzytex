...
%
%  Copyright (C) 2001, 2002, 2003, 2004, 2010, 2011, 2013, 2020, 2021,
%  2024 INRIA
%
...
A misconfiguration of your installation, or ---much more subtle--- a
...
small test file is created for which a specialized version of latex format
...
suspicious. If some windows remain opened, your configuration is likely to
be erroneous (and so, even if not detected by the script).
...
\end{tt}
\end{quote}
This assumes that \lst"whizzytex.el" has been installed in your (X)Emacs
...
as being mastered, which may not be what you intend.
\end{quote}
...
formatting or \LaTeX ing the full document, or while recompiling the current
...
calling the \texttt {Jump to error} menu entry (or the equivalent key
sequence).
...
Actually, the error overlay may just indicate that you are in the middle of
...
Whether an overlay is ephemeral and mean an incomplete edition or
persistent and mean a real {\LaTeX} error is usually unambiguous.
In addition, because \whizzy ing is dynamic and the error is reported
...
(and other processing messages) in its interaction buffer.
...
The {\tt View Log...} menu entry can be used to view the complete log files of
...
by default all log and auxiliary files will be removed at exit
...
available if an error occurred before the corresponding command has been
called.
...
check the on-line documentation of variables set or functions calls, and
...
for shell-script customization, and the format log file for latex
configuration.
...
and {\whizzy} macros, but this situation seems rather infrequent.  So there
...
you must restart {\whizzy} in debug mode (select the debug mode in the
...
below. While this mechanism is quit convenient for short and simple
...
for which you should prefer local customization files (see Section~\ref
{sec/local-custom}).
...
\end{verbatim}
It tells {\whizzy} to use \lst"iniptex" and \lst"platex" commands instead
...
being editing or other files included.  When slice delimiters are not
...
Thus, if you wish to use another previewer, you also need to customize the
...
\\
FMT & fmt & efmt & format extensions
\\
...
exceptionally during installation), and instead use system, user, or local
configuration files.
...
\subsection{Viewing with \texttt{llpp}}

\href{https://wiki.archlinux.org/index.php/Llpp}{\texttt{llpp}}
is a PDF previwer based on the \href {https://mupdf.com/}{\texttt{mupdf}}
renderer and implemented in OCaml (and C).

It allows for remote control of file reloading without blinking as well
as synctex facilities and is therefore well-suited as a PDF
{\Whizzy} previewer.

Examples with \lst"llpp" are provided in \lst"./examples/llpp/".

This is activated with the line in the source file.
\begin{verbatim}
        %; whizzy -pdf llpp
\end{verbatim}
It can also be controled in a shell configuration file by setting
\begin{verbatim}
...
For synctex use, you may copy the file \lst"./examples/llpp/llpp-synctex" in
a directory in your executable path.

\lst"llpp" currently lacks support to jump to some anchor as well as
contextual information for synctex (which it could have since it can tell
...
This is certainly the most tricky part of \WhizzyTeX.  The main idea is to
build a customized format specialized to the current document so as to avoid
reloading the whole macros at each compilation. This is almost entirely
transparent to the user, that is, the use does not have to
understand this trick. Except for two constraints:
\begin{itemize}

\item
  \lst"\begin{document}" should be typed as such   without any white white
  space; and
\item
  macros should all be defined before \lst"\begin{document}" or be local to
  a slice.
\end{itemize}
The latex source say \lst"myjob.tex" document will be loaded twice, but in
two different modes.  First, it is loaded by initex to build a format. The
package \lst"whizzytex" is loaded first before the source file
\lst"myjob.tex" is loaded with \lst"\input{\myjob.tex}".

This package redefines \lst"\document" to execute
\lst"\dump" the format to \lst"myjob.fmt" after preloading
auxiliary files, running some \WhizzyTeX hooks, and more importantly, after
redefining \lst"\documentclass" and \lst"\document" to behave differently
than in latex.

Namely, running latex with the format \lst"myjob" may behaves in two
different ways:
\begin{itemize}

\item
  When the file is a latex file that starts with \lst"\documentclass", it is
  expected to be the original file \lst"myjob.tex", and the
  \lst"\documentclass" will set the whizzytex mode to \texttt{full}
  document, skip all the code to \lst"\begin{document}" and resume there
  with the \LaTeX \lst"\document" command.

\item
  Otherwise file should start with \lst"\document" which will be executed in
  \texttt {slicing} mode.

\end{itemize}
There are only a only a few differences between \texttt{full} and
\texttt{slicing} modes, most of which  have to do with
loading of auxiliary files, adjustment of counters, decorations and user
messages.  Recompilation in full mode does more instrumentation dumps
counters at sectioning commands (chapters, sections, and subsections), which
is useful in slicing mode to correctly numbered sections and pages on
slices---as far as possible.

This structure seems robust---and seems to work with rather complex macros.
Over years, the main changes that had to be done is when latex formats was
restructured to run the body of the document in group level 0 rather than
1.\footnote {\def \lst"#1"{\texttt{\string #1}}That is, when the command
\lst"\document" started by \lst"\endgroup" to undo the \lst"\begingroup" of
\lst"\begin" in \lst"\begin\{document\}"}
...
When building the format, {\whizzy} also look for configuration files (any
file of name \lst"whizzy.sty" or \lst"myjob.wsty" in some search path),
which are loaded at the end of the \WhizzyTeX macros.  This may be used to
add other macros in {whizzy} mode, {\em e.g.}  some {\TeX} environments may
be redefined to changed they type setting, according to whether the current
line is inside or outside the environment.  (We have written such an
extension for an exercise package that sends the answers at the end in an
appendix, unless the cursor is inside the answer, in which case the answer
is in-lined.)
...
duplex}, and by default just watch for the presence of a new slice.
...
Multiple {\whizzy} running on the same file would certainly raise racing
...
% LocalWords:  SIGUSR nizes acroread localmacros ghostscript href htoc impl
% LocalWords:  WhizzyEditing plateforms Cygwin xpdf uncompress checkconfig
% LocalWords:  Makefile helpme el elc misconfiguration advertized initex ln
% LocalWords:  platex iniptex BIBTEX jbibtex bibtex subdirectory ESC emacs
% LocalWords:  LaTeX ltx auctex recompiles inlined reformats mkslice mkfile
% LocalWords:  dvicopy pdf multidisplay preprocessor recompilations idx wsh
% LocalWords:  makeindex wdvi pdflatex elatex basefile efmt ebibtex wel cfg
% LocalWords:  defaultmkslice preprocessing defaultmkfile TEXINPUTS wsty
% LocalWords:  masterbasename basenames CONFIGDIR myjob preloading stdin
